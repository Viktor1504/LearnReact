...
import { tasksActions } from '../model/tasks.reducer'
...
export const TodolistsList: React.FC<PropsType> = ({ demo = false }) => {
  ...
  const changeStatus = useCallback((taskId: string, status: TaskStatuses, todolistId: string) => {
    dispatch(tasksActions.updateTask({ taskId, model: { status }, todolistId }))
  }, [])

  const changeTaskTitle = useCallback((taskId: string, newTitle: string, todolistId: string) => {
    dispatch(tasksActions.updateTask({ taskId, model: { title: newTitle }, todolistId }))
  }, [])
  ...
  const removeTodolistCallback = useCallback((todolistsId: string) => {
    dispatch(todolistsActions.removeTodolist(todolistsId))
  }, [])
